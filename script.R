 ...
 b3 <- tq_get(b3, from = '2015-08-05',
  ...
  group_by(symbol)
    ...
    row.names(interceptos)<-c('alfa','beta')
     ...
     write_xlsx(interceptos, 'interceptos projeto ver�o.xlsx')
...
ggplot(estimativa, 
  ...
  geom_line(color= 'red')+
  labs(title = "retorno m�dio por dia", y = 'dia', 
       x='retorno m�dio')
ggsave("retorno m�dio por dia.png")
###########################################
#sem joeslay day
...
ggplot(estimativa_sjd, 
       aes(x=tempo, y = retorno_anormal_acumulado_medio)) +  
  geom_line(color= 'red')+
  labs(title = "Retorno m�dio por dia sem Joeslay day", 
       y = 'dia', x='retorno m�dio')

ggsave("Retorno m�dio por dia sem Joeslay day.png")
